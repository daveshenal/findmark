...
document.getElementById('debugCopy').addEventListener('click', () => {
  chrome.runtime.sendMessage({ type: 'GET_DEBUG' }, (response) => {
    ...
    const text = response.logs.map(l => `[${l.ts}] [${l.level}] ${l.msg}`).join('\n');
    navigator.clipboard.writeText(text).then(() => {
      document.getElementById('debugCopy').textContent = 'copied!';
      setTimeout(() => { document.getElementById('debugCopy').textContent = 'copy'; }, 1500);
    });
  });
});
  ...
  pollInterval = setInterval(() => {
    chrome.runtime.sendMessage({ type: 'STATUS' }, (response) => {
      ...
      if (response?.status === 'ready') {
        ...
        showState('📚', 'Indexing your bookmarks…', `Processing ${response.count || '...'} bookmarks.`);
